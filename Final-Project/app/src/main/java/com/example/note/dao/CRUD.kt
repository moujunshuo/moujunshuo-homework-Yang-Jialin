...
//        }
        ...
        contentValues.put(NoteDatabase.STATE, note.state)
    ...
    fun getState() : Pair<Int,Int>{
        ...
        if (cursor2.count > 0) {
            while (cursor2.moveToNext()) {
                if ( cursor2.getInt(cursor2.getColumnIndex(NoteDatabase.STATE))==0){
                    incompleteNum++
                }
                else{
                    completeNum++
        ...
        cursor2.close()
        return incompleteNum to completeNum
    }
        ...
        get() {
            ...
            if (cursor.count > 0) {
                while (cursor.moveToNext()) {
                    ...
                    note.state = cursor.getInt(cursor.getColumnIndex(NoteDatabase.STATE))
    ...
    fun updateNote(note: Note): Int {
        ...
        values.put(NoteDatabase.STATE, note.state)
